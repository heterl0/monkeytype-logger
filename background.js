...
chrome.storage.onChanged.addListener((changes, areaName) => {
  if (areaName === "local" && changes.activeSwitch) {
    ...
    console.log(
      "MonkeyType Logger: Active switch updated to:",
      activeSwitchState
    );
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  chrome.storage.local.get(["records"], (result) => {
    ...
    if (message.action === "saveRecord" && message.record) {
      console.log(
        "MonkeyType Logger: Processing saveRecord with",
        message.record.words.length,
        "words"
      );
      ...
      if (isDuplicate(message.record, records)) {
        console.log(
          "MonkeyType Logger: Duplicate record detected, skipping save"
        );
      ...
      try {
        ...
        chrome.storage.local.set({ records: records }, () => {
          if (chrome.runtime.lastError) {
            console.error(
              "MonkeyType Logger: Error saving to storage:",
              chrome.runtime.lastError
            );
            sendResponse({ success: false, reason: "storage_error" });
          } else {
            console.log(
              "MonkeyType Logger: Successfully saved record, total records:",
              records.length
            );
            ...
            if (records.length >= 10000) {
              console.log(
                "MonkeyType Logger: Record limit reached, saving to file"
              );
      ...
      if (records.length > 0) {
        ...
        chrome.storage.local.set({ records }, () => {
          console.log(
            "MonkeyType Logger: Last record deleted, remaining:",
            records.length
          );
          sendResponse({ success: true, recordCount: records.length });
        });
  ...
  reader.onload = function () {
    chrome.downloads.download(
      ...
      (downloadId) => {
        if (chrome.runtime.lastError) {
          console.error(
            "MonkeyType Logger: Download error:",
            chrome.runtime.lastError
          );
        } else {
          console.log(
            "MonkeyType Logger: File download started, ID:",
            downloadId
          );
        }
      }
