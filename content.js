...
function waitForElement(selector, callback, maxAttempts = 20) {
  ...
  if (el) {
    ...
    console.log("MonkeyType Logger: Waiting for element", selector, "attempts left:", maxAttempts);
...
function findResultsContainer() {
  ...
  for (const selector of possibleSelectors) {
    const el = document.querySelector(selector);
    if (el) {
      console.log("MonkeyType Logger: Found results container with selector:", selector);
      return el;
    }
  }
  
  // If no specific container found, look for any element with word elements
  const wordElements = document.querySelectorAll('.word, [class*="word"], .letter, [class*="letter"]');
  if (wordElements.length > 0) {
    console.log("MonkeyType Logger: Found word elements, using parent container");
...
function waitForIndividualWPMData(wordElements, maxWaitTime = 5000) {
  return new Promise((resolve) => {
    ...
    function checkForWPMData() {
      ...
      console.log("MonkeyType Logger: WPM data check - found", wpmDataCount, "words with WPM data out of", wordElements.length);
      
      // Consider data available if we have WPM for at least 25% of words or timeout reached
      const hasEnoughData = wpmDataCount > 0 && (wpmDataCount >= wordElements.length * 0.25 || (Date.now() - startTime) > maxWaitTime);
      
      if (hasEnoughData) {
        console.log("MonkeyType Logger: WPM data check completed, has sufficient data:", hasWPMData);
        resolve(hasWPMData);
      } else {
...
function extractIndividualWPM(wordElement) {
  ...
  const wpmAttr = wordElement.getAttribute("data-wpm") || 
  ...
  const wpmElement = wordElement.querySelector('[data-wpm], .wpm, [class*="wpm"]');
  if (wpmElement) {
  ...
  if (titleAttr) {
    const wpmMatch = titleAttr.match(/wpm[:\s]*(\d+(?:\.\d+)?)/i) || 
...
function getWPMFromHover(wordElement) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      ...
      const tooltips = document.querySelectorAll('.tooltip, .popup, [class*="tooltip"], [class*="popup"], [data-tooltip]');
      let wpmValue = null;
      
      for (const tooltip of tooltips) {
        const tooltipText = tooltip.textContent || tooltip.innerText || "";
        // Look for WPM patterns in tooltip text
        const wpmMatch = tooltipText.match(/wpm[:\s]*(\d+(?:\.\d+)?)/i) || 
...
function extractWPMData() {
  ...
  for (const selector of wpmSelectors) {
    ...
    if (wpmElement) {
      console.log("MonkeyType Logger: Found WPM element with selector:", selector);
      break;
    }
  }
  ...
  const summaryElements = document.querySelectorAll('.stats, .summary, .result, [class*="stat"], [class*="summary"]');
...
function initializeLogger() {
  ...
  if (resultsContainer) {
    console.log("MonkeyType Logger: Initializing observer on results container");
    ...
    console.log("MonkeyType Logger: No results container found, retrying in 1 second");
...
async function processTestResult() {
  ...
  for (const selector of possibleWordSelectors) {
    ...
    if (wordsContainer) {
      console.log("MonkeyType Logger: Found words container with selector:", selector);
      break;
    }
  }
  ...
  const wordElements = wordsContainer.querySelectorAll(".word, [class*='word'], .letter, [class*='letter']");
  ...
  console.log("MonkeyType Logger: WPM data detection completed - observer:", observerResult, "polling:", pollingResult);
  ...
  for (let i = 0; i < wordElements.length; i++) {
    ...
    word = wordElem.getAttribute("data-word") || 
    ...
    if (!word) {
      ...
      // If still no word, try to reconstruct from letter elements
      if (!word) {
        const letters = wordElem.querySelectorAll(".letter, [class*='letter'], span, div");
        if (letters.length > 0) {
          word = Array.from(letters)
    ...
    if (word.length >= 20) {
      console.log("MonkeyType Logger: Skipping long word:", word);
      continue;
    }
    ...
    if (directWPM) {
      ...
      try {
        ...
        console.log("MonkeyType Logger: Error extracting WPM from hover for word:", word, error);
        wordWPM = overallWPM;
      }
    }
    
    console.log("MonkeyType Logger: Found word:", word, "reason:", reason, "WPM:", wordWPM);
  ...
  console.log("MonkeyType Logger: Logging", wordsData.length, "words with individual WPM data");
  ...
  chrome.runtime.sendMessage({ action: "saveRecord", record }, (response) => {
    if (chrome.runtime.lastError) {
      console.error("MonkeyType Logger: Error sending message:", chrome.runtime.lastError);
    } else {
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      for (const mutation of mutations) {
        if (mutation.type === 'attributes') {
          ...
          if (mutation.attributeName === 'burst' || 
            ...
            if (wpm) {
              wpmDataFound = true;
              console.log("MonkeyType Logger: WPM data found via mutation observer on", mutation.attributeName, "attribute");
              break;
            }
          }
      ...
      if (wpmDataFound) {
        console.log("MonkeyType Logger: WPM data detected via mutation observer");
        observer.disconnect();
        resolve(true);
      }
    });
